...
1: 首页顶部双段胶囊中的“推荐”显示每日推荐卡片，“全部”显示 `RecommendationCatalog.kt` 中启用的完整推荐目录。
2: “全部”页面使用横向分类画廊，不使用普通纵向信息列表；顶部为可横向滚动的类型筛选胶囊，主体为大尺寸圆角卡片与后方露边堆叠卡。
3: 卡片只展示浅色类型背景、顶部分类标签和分类插画，不显示推荐标题与描述；已记录状态只使用标签旁的小型勾选标记。
4: 类型顺序固定为：美食、通勤、运动、工作、娱乐、社交、放松、家务、学习、探索。
5: 当前居中卡片需要更高层级、完整透明度和较明显阴影；相邻卡片缩小、下移并露出边缘，形成参考图式的专辑堆叠关系。
6: 点击非居中卡片时先滚动到该卡片；点击已居中的卡片进入对应推荐的编辑记录页。
7: 已记录状态必须通过记录的 `recommendationId` 与推荐 `id` 精确匹配，禁止通过本地化标题或分类文字推断。
8: 顶部“全部推荐”与底部“历史记录”是两个独立内容区域：前者展示推荐目录，后者展示用户已经保存的记录，禁止复用为同一页面。
9: 全部推荐 UI 独立放在 `ui/recommendations/AllRecommendationsContent.kt`，并提供 `@Preview`、中英文字符串资源和 `debugOutline()`。
10: 每日推荐与首页三张推荐卡必须优先选择尚未产生记录的推荐；未记录条目不足时，才允许使用已记录条目补齐。全部条目都已记录时，回退到按日期稳定轮转的目录顺序。
